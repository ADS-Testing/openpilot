...
if __name__ == '__main__':
    ...
    if mode == 'show_traj_map':
        ...
        for i, method_name in enumerate(method_list):
            ...
            y_axis = '# Distinct Fusion Errors'
            if y_axis == '# Fusion Errors':
                plt.errorbar(th_list, fusion_errors_count_np_mean, yerr=fusion_errors_count_np_std, label=name_mapping[method_name], color=color, linestyle='--', alpha=0.5, capsize=5)
                error_postfix = '_all'
            elif y_axis == '# Distinct Fusion Errors':
                plt.errorbar(th_list, distinct_fusion_errors_count_np_mean, yerr=distinct_fusion_errors_count_np_std, label=name_mapping[method_name], color=color, linestyle='-', alpha=0.5, capsize=5)
                error_postfix = ''
        ...
        sns.despine()
        plt.savefig(parent_folder_postfix+error_postfix+'.pdf', transparent=True, bbox_inches='tight', pad_inches=0)
