...
def draw_field(parent_folders_results, title):
    ...
    for category, results in parent_folders_results.items():
        ...
        if title == 'fusion percent error':
            ...
            # range = (-5, 15)
            range = (0, 1)
            # normalize to some extent (also offset car length)
            result = (result + 3.2) / 18
            xlabel = r'$\mathbf{F}_{d}$'
    ...
    plt.savefig('_'.join(title.split(' '))+'.pdf', transparent=True, bbox_inches='tight', pad_inches=0)
    plt.clf()
...
if __name__ == '__main__':
    ...
    # statistical test of objectives
        ...
        obj = 'min_d'
        if obj == 'fusion':
            dist1 = objectives_non_fusion[:, 5]
            dist2 = objectives_fusion[:, 5]
        elif obj == 'min_d':
            dist1 = objectives_normal[:, 0]
            dist2 = objectives_fusion[:, 0]
        # at 0.05 significance
        c_alpha = 1.358
        m = len(dist2)
        n = len(dist1)
        coeff = np.sqrt((m+n) / (m*n))
        print('m', m, 'n', n)
        print('c_alpha*coeff threshold', c_alpha*coeff)
        print("ks test = ", ss.kstest(dist2, dist1))
